...
class Master{
    ...
    function __construct(){
        $this->conn = new mysqli("localhost", "root", "", "note");
        if(!$this->conn){
    ...
    function save_note(){
        ...
        extract($_POST);
        if(empty($id)){
            $sql = "INSERT INTO `notes` (`title`, `description`,`color`) VALUES ('{$title}', '{$description}','{$color}')";
        }else{
            $sql = "UPDATE `notes` set `title` = '{$title}', `description` = '{$description}' , `color` = '{$color}' where `id` = '{$id}'";
        }
